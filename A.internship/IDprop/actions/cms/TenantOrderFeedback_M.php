...
function getpropertyid($userid)
{
	...
	$sql3= " SELECT 
		...
		INNER JOIN PropertyID ON PropertyID.ID=PropertyTermsID.Property_ID
 	 	...
 	 	WHERE ((`TenantID`.`UserType`='Tenant_All') OR (`TenantID`.`UserType`='Tenant_PM_SS') OR (`TenantID`.`UserType`='Tenant_PM') OR (`TenantID`.`UserType`='Tenant_All') ) 
 	 	AND  (`TenantID`.`User_ID`) =:userid
		";
 	$cq3 = $CONNECTION->prepare($sql3);
...
function getTenantOrderid($userid)
{
	...
	$sql3= " SELECT
 		TenantOrdersID.ID AS tenantOrdersID, 		
 	 	...
 	 	INNER JOIN PropertyTermsID ON PropertyTermsID.User_ID=TenantOrdersID.User_ID	
		WHERE  ((`TenantOrdersID`.`SupplierOrders_ID` IS NOT NULL) 
			AND (`TenantOrdersID`.`TenantFeedback` IS NULL)) 
			AND (`TenantOrdersID`.`User_ID`) = :userid
