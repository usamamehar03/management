...
function getUserRates()
{
	global $CONNECTION;
	$out = FALSE;	
	$sql4= "SELECT supplierfee.Supplier_ID, servicetype.Type, supplierfee.CallOutCharge, supplierfee.BillingIncrement,supplierfee.HourlyRate, supplierfee.OvertimeRate, supplierfee.WeekendRate, supplier.SupplierHasFixed
	from SupplierFeesID as supplierfee inner join SupplierID as supplier 
	ON supplierfee.Supplier_ID = supplier.ID
	left join MaintenanceTypeID as servicetype 
	ON supplierfee.MaintenanceType_ID= servicetype.ID
	left join SupplierFeesApprovalID as feeaproval 
	ON supplier.ID= feeaproval.Supplier_ID WHERE feeaproval.Supplier_ID IS NULL
	order by supplierfee.ID";
	$cq4 = $CONNECTION->prepare($sql4);
	if( $cq4->execute() ){
