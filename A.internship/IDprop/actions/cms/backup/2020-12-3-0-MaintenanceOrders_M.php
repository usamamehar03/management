...
function isExistMaintenanceType($supplier,$maintenancetype)
{
	global $CONNECTION;
	$out = FALSE;
	$sql3= " SELECT MaintenanceType_ID
	 	from MaintenanceOrdersID 
	 	WHERE Supplier_ID=:supplier and MaintenanceType_ID=(SELECT ID FROM MaintenanceTypeID where Type=:maintenancetype)
	";
	$cq3 = $CONNECTION->prepare($sql3);
...
function selectHourlySupplier($data)
{
	...
	$sql3= "SELECT DISTINCT SupplierFeesID.Supplier_ID AS Supplier_ID,
		SupplierFeesID.$type AS HourlyRate, 
	 	($type+BillingIncrement) AS cheap,
