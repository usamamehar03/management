...
function getUserRates($propertymanagementid)
{
	...
	from SupplierFeesID as supplierfee 
		...
		INNER JOIN SupplierStaffID ON supplierfee.Supplier_ID=SupplierStaffID.Supplier_ID
		INNER JOIN PaymentClientID ON PaymentClientID.User_ID=SupplierStaffID.User_ID
		INNER JOIN PropertyManagementID ON PaymentClientID.PropertyManagement_ID=PropertyManagementID.ID
	WHERE feeaproval.Supplier_ID IS NULL 
