...
function getsupllierfixedjobdata($supplier_id)
{
	...
	$sql3= "SELECT distinct
		...
		INNER JOIN SupplierID ON SupplierID.ID=SupplierOrdersID.Supplier_ID
		...
		And TenantOrdersID.SupplierOrders_ID IS NULL
...
function addSupplierInvoice($todaydate,$user_id,$supplierid,$data)
{
	global $CONNECTION;
	$out = FALSE;
	$sql3= "INSERT INTO InvoiceID (User_ID, PropertyManagement_ID, Supplier_ID, MaintenanceOrder_ID, Property_ID, InvoiceNumber, InvoiceDate, DueDate)
	VALUES (:user_id, :propertyManager_id, :supplierid, :maintenanceorders_id, :property_id, :invoicenumber,:invoicedate, :dueDate)";
	$cq3 = $CONNECTION->prepare($sql3);
...
function addInvoiceGroupid($invoice_id, $invoicedetails_id, $itempart_id,$partprice)
{
	global $CONNECTION;
	$out = FALSE;
	$sql3= "INSERT INTO InvoiceGroupID (Invoice_ID, InvoiceDetails_ID, ItemParts_ID, Amount)
	VALUES (:invoice_id, :invoicedetails_id, :itemparts_id, :amount) ";
	$cq3 = $CONNECTION->prepare($sql3);
	$cq3->bindValue(':invoice_id',$invoice_id);	
	$cq3->bindValue(':invoicedetails_id',$invoicedetails_id);
	$cq3->bindValue(':itemparts_id',$itempart_id);
	$cq3->bindValue(':amount',$partprice);		
	if( $cq3->execute() ){
