...
//Sara add 2 more functions after select property show tenant and show owner
...
//Tenant name and address for NEW invoice.
...
//Owner company name and address for NEW invoice
// getOneOwnerProperty(640000000,341);
...
//Tenant name for each storage rental unit. NEW Invoice
function getStorageUnitTenant($propertyManagementid,$storagefacilityid)
{
	global $CONNECTION;
	$out = FALSE;
	$sql= "SELECT 'tenant' as type,
	...
	WHERE 
	StorageRentalsID.Tenant_ID=TenantID.ID
...
//Owner name for each storage rental unit. NEW Invoice
function getStorageUnitOwner($propertyManagementid,$storagefacilityid){
	global $CONNECTION;
	$out = FALSE;
	$sql= "SELECT 'owner' as type,
