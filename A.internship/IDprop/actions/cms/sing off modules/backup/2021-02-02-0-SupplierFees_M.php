...
function isItemTypeExist($supplierid,$maintenanceid,$data)
{
	global $CONNECTION;
	$out = FALSE;
	$sql3= "SELECT  SupplierFixedLabourRatesID.ID as id
			FROM SupplierFixedLabourRatesID
			INNER JOIN ItemTypeID ON SupplierFixedLabourRatesID.ItemType_ID=ItemTypeID.ID
			where SupplierFixedLabourRatesID.Supplier_ID=:supplier_id and ItemTypeID.MaintenanceType_ID=
			:maintenanceid and ItemTypeID.ItemType=:itemtype
			";
	$cq3 = $CONNECTION->prepare($sql3);
	$cq3->bindValue(':supplier_id',$supplierid);
	$cq3->bindValue(':maintenanceid',$maintenanceid);
	$cq3->bindValue(':itemtype',$data['itemType1']);
	if( $cq3->execute() ){
...
function issupplierfeesidexist($supplier_id,$maintenanceType_ID)
{
	global $CONNECTION;
	$out = FALSE;
	$sql3= "SELECT  Supplier_ID FROM SupplierFeesID 
		where Supplier_ID=:supplierid and MaintenanceType_ID=:maintenanceType_ID";
	$cq3 = $CONNECTION->prepare($sql3);
	$cq3->bindValue(':supplierid',$supplier_id);
	$cq3->bindValue(':maintenanceType_ID',$maintenanceType_ID);
	if( $cq3->execute() ){
...
function getSupplierfeesid($id)
{
	...
	ORDER BY `SupplierFeesID`.`ID` DESC LIMIT 1
	";
...
function getsupplierid($userid)
{
	global $CONNECTION;
	$out = FALSE;
	$sql3= "SELECT Supplier_ID from SupplierStaffID where User_ID=:userid";
	$cq3 = $CONNECTION->prepare($sql3);
...
function getSupplierUserId($id){
	...
	WHERE (`SupplierStaffID`.`User_ID`) AND (`SupplierStaffID`.`UserRole`=`Supplier_SM`)= :user
	";
	$cq3 = $CONNECTION->prepare($sql3);
	$cq3->bindValue(':user',$id);
	if( $cq3->execute() ){
