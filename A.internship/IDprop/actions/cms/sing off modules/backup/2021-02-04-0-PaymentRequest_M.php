...
// $res=tenant_lettingagentclient_list(1000000613);
// foreach ($res as $key => $value){
// 	print_r($value);
// 	echo "</br>";
	
// }
function propertmanagmentclient_list($user_id)
{
	$joins=data_joins('PaymentClientID.User_ID');
	global $CONNECTION;
	$out =FALSE;
 	$sql = "SELECT PaymentClientID.User_ID as paymentclient_id,
 		...
 		$joins
 		WHERE PaymentClientID.User_ID!=:user_id 
 		AND (UserID.EndUser BETWEEN 640000000 AND  649999999) 
 	";
	$cq = $CONNECTION->prepare($sql);
	$cq->bindValue(':user_id',$user_id);
	if( $cq->execute() ){
...
// $res=propertmanagmentclient_list(1000000611);
// foreach ($res as $key => $value){
// 	print_r($value);
// 	echo "</br>";
...
function data_joins($userid)
{
	$joins="INNER JOIN UserID ON UserID.User_ID=$userid
 		...
 		LEFT JOIN SupplierID ON  SupplierID.User_ID=$userid
 		";
 		return $joins;
}
...
// {
	...
	// global $CONNECTION;
	// $out =FALSE;
 // 	$sql = "SELECT PaymentClientID.User_ID as paymentclient_id, EndUser,
 // 		CASE 
 // 		WHEN UserID.EndUser BETWEEN 875000000 AND 949999999
	//  		THEN 
	//  			CONCAT(
