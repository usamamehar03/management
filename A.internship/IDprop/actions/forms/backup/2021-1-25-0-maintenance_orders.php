...
{
	$id=640000000;
	$propertyID=maintenanceOrders\getpropertyid(640000000);
	if($propertyID!=false || !empty($propertyID))
	{
		...
		foreach ($propertyID as $key1 => $value1)
		{
			foreach ($propertyID_buildingNames as $key2 => $value2)
			{
				...
				{
					// echo $propertyID[$key1]['building'];
					$propertyID_buildings[$value2][]=array('propertyid'=>$propertyID[$key1]['propertyid'],'firstline' => $propertyID[$key1]['firstline'],'city' => $propertyID[$key1]['city'],'country'=> $propertyID[$key1]['country'],'postcode'=>$propertyID[$key1]['postcode']);
					break;
				}
			}
		}
	...
	{
		echo json_encode(['status'=>$propertyID],JSON_FORCE_OBJECT);
	}
