define(['knockout', 'jquery', 'moment', 'bootstrap', 'notify', 'modal', 'timedUpdate'], function (ko,$,moment){
	...
	function maintenanceOrdersViewModel() {
			...
			.done(function( data ) {
				...
				if( data ){
					...
					else
					{
						...
						{
							alert("you want another supplier for same job ");
							self.isdecision(true);
							self.decisionmsg('you want another supplier for same job?');
						}
		...
		function getpropertyid(){
			var d = $.Deferred();
			$.post( '../actions/forms/maintenance_orders.php', { 'act':'getpropertyid'})
			.done(function( data ) {
				data=JSON.parse(data);
				if( data ){
					if( data.status == 'ok' ){
