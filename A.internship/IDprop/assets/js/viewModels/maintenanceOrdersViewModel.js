define(['knockout', 'jquery', 'moment', 'bootstrap', 'notify', 'modal', 'timedUpdate'], function (ko,$,moment){
	...
	$(".errorhandler").on("change", function(){
	...
	function maintenanceOrdersViewModel() {
	    ...
	    selectedbuildig.subscribe(function (vale){
	    	if (vale!=null)
	    	{
	    		...
	    		setTimeout(hourlyresponse,100,$(this).val());
	    	}
	    	else
	    	{
	    		self.requestedbuildingname('no building');
	    		$('#building_ID').next(".error").html("");
				$('#building_ID').attr('style', 'background-color: white !important');
				var data=self.buildingrawdata()['no building']!=undefined?self.buildingrawdata()['no building']:null
				...
				if( data ){
					...
					else
					{
						...
						else
						{
							...
							{
								$("#"+key).html('empty');
								// $("#"+key).siblings('.invo_input,errorhandler').css({"background-color": "#f8d7da"});
								$("#"+key).siblings('.errorhandler').attr('style', 
									'background-color: #f8d7da !important');
							 	$("#"+key).siblings('.errorhandler').children().css({
		...
		{
			self.propertylist.removeAll();
			propertyidcount =name?Object.keys(name).length:0;
		...
		// function displayAllPropertylist()
		// {
		// 	self.propertylist.removeAll();
		// 	data=self.buildingrawdata();
		// 	propertyidcount = Object.keys(data).length;
		// 	if (propertyidcount>0)
		// 	{
		// 		for (var key of Object.keys(self.buildingrawdata()))
		// 		{
		// 			for (var key2 of Object.keys(data[key]))
		// 			{
		// 				index=data[key][key2];
		// 				self.propertyname=ko.observable(index.firstline+',  '+
		// 					index.city+',  '+index.country+',  '+index.postcode);
		// 				self.propertyid=ko.observable(index.propertyid);
		// 				self.addproperty();
		...
		function displaybuilding()
		{
			...
			.done(function(data){
				...
				displaypropertylist(self.buildingrawdata()['no building']);
			})
			// self.buildinglist.removeAll();
			// buildingcount = Object.keys(data).length;
			// if (buildingcount>0)
