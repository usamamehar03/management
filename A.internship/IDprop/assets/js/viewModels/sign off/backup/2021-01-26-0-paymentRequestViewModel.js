define(['knockout', 'jquery','moment','bootstrap','notify','modal'], function (ko,$,moment){
	$(".invo_input, .invo_select").on("input", function(){
		if(($(this).val()!="" && $(this).val()!="type"))
		{
			...
			if($(this).is(':disabled'))
			{
				$(this).attr('style', 'background-color: #f4f5f7 ');
			}
			else
			{
	...
	function paymentRequestViewModel() {
		...
		self.add_client=function(id,name)
		{
			self.clientlist.push({id:id, name:name});
		}
			...
			.fail(function(data){
				if (data.status=='err')
				{
					for (var key of Object.keys(data.data))
					{
						...
						{
							$("#"+key).html('letters and numbers only');
						}
		...
		*/
			...
			setTimeout(function(){
				if (self.selectedinvoice()!=undefined)
				{
					...
					.done(function(data){
						...
						self.selectedclient(null);
						self.purpose(data[0].service);
						self.notes(data[0].description);
						$(".invo_input, .invo_select").trigger('input');
						//self.ChangeDisabledColor(".disable");
					})
					.fail(function(){
						self.loaddata();
						self.selectedclient(null);
		...
		.done(function(data){
			...
			if (count>0)
			{
				...
				while(i<count)
				{
					self.add_client(data[i].paymentclient_id, data[i].name);
					i++;
				}
			}
		})
