define(['knockout', 'jquery', 'moment', 'bootstrap', 'notify', 'modal', 'timedUpdate'], function (ko,$,moment){
	$(document).on("input", ".invo_input, .invo_select", function(){
	...
	function supplierFinalViewModel() {		
		...
		function displayorder(property_id=null,propertymanagmentid=null,userid=null,
			supplierorderid=null,maintenanceorderid=null,rate=null,price=null,
			calloutcharge=null,billingincrement=null)
		{
		...
		self.displayparts=function()
		{
			...
			if (count>0)
			{
				...
				{
					self.addsupplierpart(index['parts'][key].materialcostid,
						index['parts'][key].partname, index['parts'][key].partprice,
						null,null,'serialnumber'+key+'Error','warranty'+key+'Error');
				}
			}
		}
		self.loadorder=function()
		{
			...
			displayorder(property_id, index.property_adress.propertymanagmentid,
				index.property_adress.userid, index.supplierorder_id, 
				index.maintenanceorders_id, index.rate, index.price, calloutcharge,
				billingincrement);
			self.displayparts();
