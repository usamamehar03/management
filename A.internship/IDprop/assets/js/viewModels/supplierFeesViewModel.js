...
eraseErrors();
	function supplierFeesViewModel() {
		...
		{
			var d = $.Deferred();
			$.post( '../actions/forms/supplier_fees.php', { 'act':'getUserID','FORM_TOKEN':FORM_TOKEN})
			.done(function( data ) {
				alert(data);
			...
			.done(function( data ) {
				...
				if( data )
				{
					...
					else
					{
						...
						if(data.status == 'fail' )
						{
							alert("supplierfee already exist or fail entry");
						}
						else
						{
							var index="_alert";
							for (var key of Object.keys(data.data)) 
							{
								...
								$("#"+key+index).siblings('.data_input').css({"background-color": "#f8d7da"});
								$("#"+key+index).siblings('.data_input').attr('style', 'background-color: #f8d7da !important');
								$("#"+key+index).siblings('.data_input').children().css({"background-color": "white"});
							}
						}
						d.reject();
					}
