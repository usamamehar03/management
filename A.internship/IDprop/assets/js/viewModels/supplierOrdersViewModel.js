define(['knockout', 'jquery', 'moment', 'bootstrap', 'notify', 'modal', 'timedUpdate'], function (ko,$,moment){
	$(".input_data").on("focusout", function(){
	...
	function supplierOrdersViewModel() {
		self.Tenantlist=ko.observableArray([{name:null,label:'Tenant Name & Mobile'}]);
		self.addTenant=function(name=null,label='Tenant Name & Mobile')
		{
			self.Tenantlist.push({name:name, label:label});
		}
		//dusplay supplier
		...
		function DisplaySupplierOrders(supplierid=null,id=null,companyname=null,
			...
			schedule=null,property_id=null)
		{
		...
		self.loaddata=function()
		{
			...
			var property_id=buildingname+index.firstline+', '+index.city
			...
			DisplaySupplierOrders(index.supplierid,index.id,index.companyname,
				...
				index.schedule,property_id);
			//load tenant

			self.getTenants(index.id?index.id:null);
		}
		//get supllier data
		...
		// get name
		self.getTenants=function(maintenanceorder_id)
		{
			alert('here');
			var callstate={
				'state':'getname',
				'maintenanceorder_id': maintenanceorder_id
			}
			getSupplierOrders(callstate)
			.done(function(data){
				count = Object.keys(data).length;   //get supplier length
				if (count>0)
				{
					// remove tenant then add
					self.Tenantlist.removeAll();
					for (var key of Object.keys(data))
					{
						var name=data[key].salutation+' '+data[key].firstname+' '
						+data[key].surname+', '+data[key].mobile;
						indexe=parseInt(key)+1;
						var label='Tenant '+indexe+' Name & Mobile';
						self.addTenant(name,label);
					}
				}
				else
				{
